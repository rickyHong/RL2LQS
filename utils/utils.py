...
class LogDataPlotter:
    def __init__(self, result_log: LogData, result_folder, json_folder_name, img_save_interval):
        self.result_log = result_log
        self.result_file_prefix = '{}/latest'.format(result_folder)
        self.img_folder = '{}/img'.format(result_folder)
        self.json_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), json_folder_name)
        self.img_save_interval = img_save_interval

        dirname = os.path.dirname(self.result_file_prefix)
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        if not os.path.exists(self.img_folder):
            os.makedirs(self.img_folder)

    def plot(self, epoch, labels=None, json_filename=None, jpg_name=None):

        # Read json
        if json_filename is None:
            # Default json
            json_filename = 'style_none'
        log_image_config_file = os.path.join(self.json_folder, json_filename)
        ...
        if labels is None:
            labels = self.result_log.get_keys()
        for label in labels:
            if self.result_log.has_key(label):
                plt.plot(*self.result_log.getXY(label), label=label)
        ...
        if jpg_name is None:
            jpg_name = '_'.join(labels)
        fig = plt.gcf()
        fig.savefig('{}-{}.jpg'.format(self.result_file_prefix, jpg_name))

        if (epoch % self.img_save_interval) == 0:
            img_file_prefix = '{}/ep{}'.format(self.img_folder, epoch)
            fig.savefig('{}-{}.jpg'.format(img_file_prefix, jpg_name))
...
def util_save_data_list_plot(jpg_filename, img_params, data_list, label=None):
    assert type(data_list) == list, 'use LIST for data'

    dirname = os.path.dirname(jpg_filename)
    if not os.path.exists(dirname):
        os.makedirs(dirname)
