...
class QTrainer:
    def __init__(self,
        ...
        if self.esmode == 'static' or self.esmode == 'manual':
    ...
    def run(self):
        ...
        while episode < total_episodes:
            ...
            for i in range(num_process-1, -1, -1):
                ...
                process = mp.Process(target=self.distributed_ES, args=(q_list[i], rq_list[i], i, cuda_num, epi_num, batch_dist, traj_ratio,
    ...
    #########################
    @staticmethod
    def distributed_ES(q, rq, id, cuda_num, epi_num, batch_dist, traj_ratio,
        ...
        Evolution_strategy.set_pauli_operators()
        cnt, fidelity, terminal_loop, mem_dict = Evolution_strategy.evolve(batch_dist, target_success, epi_num, result_folder, model, cuda_num, traj_ratio)
